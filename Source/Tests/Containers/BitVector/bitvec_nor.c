 ...
 *
 * @brief Unit test for bitvec_nor in BitVector container.
 * */
...
TEST_FN Bool Nor_WHEN_LENGTH_IS_NOT_ALIGED_TO_8BIT() {
    ...
    BitVector* bvres = bitvec_nor(bv1, bv2);
    ...
    /* test bvres */
    TEST_CONTENTS(bvres->data[0] = NOR(m11, m21));
    TEST_CONTENTS(bvres->data[1] = NOR(m12, m22));
...
TEST_FN Bool Nor_WHEN_LENGTH_IS_ZERO() {
    BitVector* bv1 = bitvec_create();
    BitVector* bv2 = bitvec_create();
    TEST_OBJECT(bv1 && bv2);
...
TEST_FN Bool Nor_WHEN_LENGTH_IS_NON_ZERO_NOT_EQUAL() {
    BitVector* bv1 = bitvec_create();
    BitVector* bv2 = bitvec_create();
    TEST_OBJECT(bv1 && bv2);
    ...
    TEST_CONTENTS(is_memory_filled_with_byte(bvres->data, minsz, TO_UINT8(NOR(0xa0, 0x0a)))); /* ~0xaa */
    TEST_CONTENTS(is_memory_filled_with_byte(bvres->data + minsz, maxsz - minsz, TO_UINT8(NOR(0xa0, 0x00)))); /* 0xa0 */
...
TEST_FN Bool Nor_WHEN_LENGTH_IS_EQUAL() {
    BitVector* bv1 = bitvec_create();
    BitVector* bv2 = bitvec_create();
    TEST_OBJECT(bv1 && bv2);
    ...
    /* test bvres */
    TEST_CONTENTS(is_memory_filled_with_byte(bvres->data, sz, TO_UINT8(NOR(0xbb, 0x22)))); /* 0xbb */
...
BEGIN_TESTS(bitvec_nor)
    TEST(Nor_WHEN_LENGTH_IS_NOT_ALIGED_TO_8BIT),
    TEST(Nor_WHEN_LENGTH_IS_ZERO),
    TEST(Nor_WHEN_LENGTH_IS_NON_ZERO_NOT_EQUAL),
    TEST(Nor_WHEN_LENGTH_IS_EQUAL)
END_TESTS()
