...
TEST_FN Bool Nor_WHEN_LENGTH_IS_NON_ZERO_NOT_EQUAL() {
    ...
    memset(bv1->data, 0x99, DIV8(sz)); /* set some data to check later */
    ...
    /* check bvres */
    for(Size s = 0; s < DIV16(sz); s++) {
        TEST_CONTENTS(bvres->data[s] == (Uint8)NOR(0x99, 0x11)); /* 0x66 */
    }
...
TEST_FN Bool Nor_WHEN_LENGTH_IS_EQUAL() {
    ...
    resizebv(bv1, sz);
    resizebv(bv2, sz);
    memset(bv1->data, 0x88, DIV8(sz));
    memset(bv2->data, 0x11, DIV8(sz));
    ...
    /* check bvres */
    for(Size s = 0; s < DIV8(sz); s++) {
        TEST_CONTENTS(bvres->data[s] == (Uint8)NOR(0x88, 0x11)); /* 0x66 */
    }
    for(Size s = sz; s < DIV8(bv1->capacity); s++) {
        TEST_CONTENTS(bvres->data[s] == 0x00);
    }
