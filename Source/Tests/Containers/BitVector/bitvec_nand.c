 ...
 * See the License for the specific language governing permissions nand
 * limitations under the License.
 *
 * @brief Unit test for bitvec_nand in BitVector container.
 * */
...
TEST_FN Bool Nand_WHEN_LENGTH_IS_NOT_ALIGED_TO_8BIT() {
    ...
    Uint8 m21 = CREATE_MASK8(1, 1, 0, 1, 0, 0, 0, 1);
    Uint8 m22 = CREATE_MASK8(0, 1, 1, 0, 0, 0, 0, 0);
    ...
    BitVector* bvres = bitvec_nand(bv1, bv2);
    ...
    /* test bvres */
    TEST_CONTENTS(bvres->data[0] = NAND(m11, m21));
    TEST_CONTENTS(bvres->data[1] = NAND(m12, m22));
...
TEST_FN Bool Nand_WHEN_LENGTH_IS_ZERO() {
    BitVector* bv1 = bitvec_create();
    BitVector* bv2 = bitvec_create();
    TEST_OBJECT(bv1 && bv2);
...
TEST_FN Bool Nand_WHEN_LENGTH_IS_NON_ZERO_NOT_EQUAL() {
    BitVector* bv1 = bitvec_create();
    BitVector* bv2 = bitvec_create();
    TEST_OBJECT(bv1 && bv2);
    ...
    bv2->length = len2;
    memset(bv1->data, 0xaa, sz1); /* set some data to check later */
    memset(bv2->data, 0x0a, sz2); /* set some data to check later */

    /* xor of 16 bit value with 32 bit value, where one is 0xaaaaaaaa nand other is 0x0000 */
    BitVector* bvres = bitvec_nand(bv1, bv2);
    ...
    /* check bv1 */
    TEST_CONTENTS(is_memory_filled_with_byte(bv1->data, sz1, 0xaa));
    ...
    TEST_CONTENTS(is_memory_filled_with_byte(bvres->data, minsz, NAND(0xaa, 0x0a))); /* ~0xaa */
    TEST_CONTENTS(is_memory_filled_with_byte(bvres->data + minsz, maxsz - minsz, NAND(0xaa, 0x00))); /* 0xff */
...
TEST_FN Bool Nand_WHEN_LENGTH_IS_EQUAL() {
    BitVector* bv1 = bitvec_create();
    BitVector* bv2 = bitvec_create();
    TEST_OBJECT(bv1 && bv2);
    ...
    bv1->length = len;
    bv2->length = len;
    memset(bv1->data, 0xbb, sz);
    memset(bv2->data, 0x22, sz);
    ...
    /* check bv1 */
    TEST_CONTENTS(is_memory_filled_with_byte(bv1->data, sz, 0xbb));
    TEST_CONTENTS(is_memory_filled_with_byte(bv1->data + sz, bitvec_get_capacity_in_bytes(bv1) - sz, 0x00));
    /* check bv2 */
    TEST_CONTENTS(is_memory_filled_with_byte(bv2->data, sz, 0x22));
    TEST_CONTENTS(is_memory_filled_with_byte(bv2->data + sz, bitvec_get_capacity_in_bytes(bv2) - sz, 0x00));
    /* test bvres */
    TEST_CONTENTS(is_memory_filled_with_byte(bvres->data, sz, NAND(0xbb, 0x22))); /* ~0x22 */
...
BEGIN_TESTS(bitvec_nand)
    TEST(Nand_WHEN_LENGTH_IS_NOT_ALIGED_TO_8BIT),
    TEST(Nand_WHEN_LENGTH_IS_ZERO),
    TEST(Nand_WHEN_LENGTH_IS_NON_ZERO_NOT_EQUAL),
    TEST(Nand_WHEN_LENGTH_IS_EQUAL)
END_TESTS()
