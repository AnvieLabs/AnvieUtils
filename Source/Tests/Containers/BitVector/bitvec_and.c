 ...
 *
 * @brief Unit test for bitvec_and in BitVector container.
 * */
...
TEST_FN Bool And_WHEN_LENGTH_IS_NOT_ALIGED_TO_8BIT() {
    ...
    BitVector* bvres = bitvec_and(bv1, bv2);
    ...
    /* test bvres */
    TEST_CONTENTS(bvres->data[0] = AND(m11, m21));
    TEST_CONTENTS(bvres->data[1] = AND(m12, m22));
...
TEST_FN Bool And_WHEN_LENGTH_IS_ZERO() {
    BitVector* bv1 = bitvec_create();
    BitVector* bv2 = bitvec_create();
    TEST_OBJECT(bv1 && bv2);
...
TEST_FN Bool And_WHEN_LENGTH_IS_NON_ZERO_NOT_EQUAL() {
    BitVector* bv1 = bitvec_create();
    BitVector* bv2 = bitvec_create();
    TEST_OBJECT(bv1 && bv2);
    ...
    TEST_CONTENTS(is_memory_filled_with_byte(bvres->data, minsz, AND(0xaa, 0x0a))); /* 0x0a */
    TEST_CONTENTS(is_memory_filled_with_byte(bvres->data + minsz, maxsz - minsz, AND(0xaa, 0x00))); /* 0x00 */
...
TEST_FN Bool And_WHEN_LENGTH_IS_EQUAL() {
    BitVector* bv1 = bitvec_create();
    BitVector* bv2 = bitvec_create();
    TEST_OBJECT(bv1 && bv2);
    ...
    /* test bvres */
    TEST_CONTENTS(is_memory_filled_with_byte(bvres->data, sz, AND(0xbb, 0x22))); /* 0x22 */
...
BEGIN_TESTS(bitvec_and)
    TEST(And_WHEN_LENGTH_IS_NOT_ALIGED_TO_8BIT),
    TEST(And_WHEN_LENGTH_IS_ZERO),
    TEST(And_WHEN_LENGTH_IS_NON_ZERO_NOT_EQUAL),
    TEST(And_WHEN_LENGTH_IS_EQUAL)
END_TESTS()
